    ...
    >
      <div className="w-full max-w-2xl rounded-2xl bg-white dark:bg-[#18181b] border border-[#e5e1d7] dark:border-[#27272a] p-6 sm:p-7 shadow-2xl space-y-5 max-h-[90vh] overflow-y-auto overflow-x-hidden animate-scale-in">
        ...
        <div className="flex items-center justify-between border-b border-[#e5e1d7] dark:border-[#27272a] pb-3">
          <div>
            <h2 className="text-lg font-semibold text-[#232f26] dark:text-[#f4f4f5]">Edit Habit</h2>
            <p className="mt-0.5 text-xs text-[#737970] dark:text-[#a1a1aa]">Update name, goal, schedule, or label</p>
          </div>
          <button
            onClick={onClose}
            aria-label="Close"
        ...
        <form onSubmit={handleSave} className="space-y-4 text-xs">
          ...
          <div>
            ...
            <input
              ...
              className="mt-1 w-full rounded-xl border border-[#e5e1d7] bg-white dark:border-[#27272a] dark:bg-[#27272a] p-2.5 text-sm text-[#232f26] dark:text-[#f4f4f5] outline-none focus:border-[#232f26]/40 dark:focus:border-[#3f3f46]"
            />
          </div>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <div>
              <label className="font-semibold text-[#232f26] dark:text-[#f4f4f5]">Label</label>
              <input
                type="text"
                value={userLabel}
                onChange={(e) => setUserLabel(e.target.value.slice(0, 30))}
                className="mt-1 w-full rounded-xl border border-[#e5e1d7] bg-white dark:border-[#27272a] dark:bg-[#27272a] p-2.5 text-[#232f26] dark:text-[#f4f4f5] outline-none focus:border-[#232f26]/40 dark:focus:border-[#3f3f46]"
              />
            </div>
            <div>
          ...
          {/* Schedule */}
          <div>
            <label className="font-semibold text-[#232f26] dark:text-[#f4f4f5]">Schedule</label>
          ...
          {freqKind !== "timesPerWeek" && (
            <div className="flex items-center gap-3">
              <label className="font-semibold text-[#232f26] dark:text-[#f4f4f5]">Allowed misses / week:</label>
              <CustomSelect
          ...
          <div className="flex justify-end gap-2 border-t border-[#e5e1d7] dark:border-[#27272a] pt-4">
            <button
              type="button"
              onClick={onClose}
              className="rounded-xl border border-[#e5e1d7] bg-white dark:border-[#27272a] dark:bg-[#18181b] px-4 py-2 font-semibold text-[#737970] dark:text-[#a1a1aa] transition-colors hover:text-[#232f26] dark:hover:text-[#f4f4f5]"
            >
              Cancel
            </button>
            <button
              type="submit"
              disabled={saving}
              className="rounded-xl bg-[#232f26] px-5 py-2 font-semibold text-white dark:bg-[#27272a] dark:text-[#f4f4f5] dark:border dark:border-[#3f3f46] transition-opacity hover:opacity-90 disabled:opacity-40"
            >
