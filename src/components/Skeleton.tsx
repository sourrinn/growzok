...
export function SkeletonProtocolCard({ delayClass = "" }: { delayClass?: string }) {
  return (
    <div
      className={`flex flex-col justify-between rounded-2xl border border-[#e5e1d7] bg-white dark:border-[#27272a] dark:bg-[#18181b] p-6 shadow-sm space-y-4 animate-fade-in ${delayClass}`}
    >
...
export function SkeletonStatTile({ delayClass = "" }: { delayClass?: string }) {
  return (
    <div
      className={`rounded-2xl border border-[#e5e1d7] bg-white dark:border-[#27272a] dark:bg-[#18181b] p-5 shadow-sm space-y-3 animate-fade-in ${delayClass}`}
    >
      <Skeleton className="h-3 w-24" />
