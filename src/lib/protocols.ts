...
export type { Protocol, ProtocolHabit, ProtocolCategory, ProtocolDifficulty } from "@/types/protocol";

export const STANDARD_PROTOCOLS: Protocol[] = [
  {
...
Context switching destroys cognitive capacity. This protocol enforces strict boundaries around high-yield work blocks.
...
- **Zero Phone First Hour**: Protects your proactive mindset before reactive inputs hijack attention.
  ...
  },
  {
    ...
    description:
      "A balanced, approachable daily movement protocol combining resistance training, daily steps, and mobility work.",
    overviewMarkdown: `
...
Intensity without consistency leads to burnout. This 30-day program focuses on daily physical momentum.
  ...
  },
  {
    ...
    name: "Software Engineer Growth Protocol",
...
export const HABIT_TEMPLATES = STANDARD_PROTOCOLS;

/** Look up a protocol by its key or slug. */
export function getProtocolByKey(key: string): Protocol | undefined {
  return STANDARD_PROTOCOLS.find((p) => p.key === key || p.slug === key);
}

export const getTemplateByKey = getProtocolByKey;

/** Filter protocols by Protocol Category. */
export function getProtocolsByCategory(category: ProtocolCategory | "All"): Protocol[] {
  if (category === "All") return STANDARD_PROTOCOLS;
  return STANDARD_PROTOCOLS.filter((p) => p.category === category);
}

export const getTemplatesByCategory = getProtocolsByCategory;

/** Get the unique set of Protocol Categories present in the catalog. */
